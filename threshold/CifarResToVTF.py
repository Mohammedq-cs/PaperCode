...
def apply_condition(input_tensor: torch.tensor, thresholdValue: float) -> torch.tensor:
...
def below_threshold_count(input_tensor: torch.tensor, thresholdValue: float) -> torch.tensor:
    condition = (thresholdValue >= input_tensor) & (input_tensor >= -thresholdValue)
...
for eps in epsilons:
    ...
    if eps < 32:
        steps = torch.arange(0, ((np.ceil(eps / 255 / 0.01))*0.01) + 0.005, 0.0025)
    else:
        steps = torch.arange(0, ((np.ceil(eps / 255 / 0.01))*0.01) + 0.01, 0.005)
    ...
    # prob Calc
    ele_count = [0 for i in range(len(steps))]
    values_sum = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        values_sum += inputs.numel()
        for index, threshold in enumerate(steps):
            admixFiltered = apply_condition(admixPest, threshold)
            vmiFiltered = apply_condition(vmiPest, threshold)
            apgdFiltered = apply_condition(apgdPest, threshold)
            ...
            # prob calc
            ele_count[index] += below_threshold_count()
