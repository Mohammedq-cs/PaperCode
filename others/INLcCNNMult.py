...
for coef in coefs:
    ...
    admixAccList = []
    vmiAccList = []
    admixAttack = AdmixAttackMult(model=imagenetteLinear, image_width=160, image_height=160, image_resize=180, momentum=1.0)
    vmiAttack = VMIAttackMult(model=imagenetteLinear, momentum=1.0, beta=1.5)
    for eps in epsilons:
        #print("eps=", eps)
        admixCorrect = 0
        vmiCorrect = 0
