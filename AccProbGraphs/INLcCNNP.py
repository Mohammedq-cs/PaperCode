...
admixAttack = AdmixAttack(model=imagenetteLinear, image_width=160, image_height=160, image_resize=180)
vmiAttack = VMIAttack(model=imagenetteLinear)
print("starting experiment")

for eps in epsilons:
    apgdAttack = APGDAttack(predict=imagenetteLinear, eps=eps / 255, device=device, is_Vt=False)
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        for index, threshold in enumerate(steps):
            ...
            correctAPGDBBFilteredAdvEps, batch_size = computeAcc(imagenetteCNN, apgdAdvFEps, targets, is_Vt=False)
            correctAdmixBBFilteredAdvEps, batch_size = computeAcc(imagenetteCNN, admixAdvFEps, targets, is_Vt=False)
            correctVMIBBFilteredAdvEps, batch_size = computeAcc(imagenetteCNN, vmiAdvFEps, targets, is_Vt=False)
            ...
            correctAPGDWBFilteredAdvEps, batch_size = computeAcc(imagenetteLinear, apgdAdvFEps, targets, is_Vt=False)
            correctAdmixWBFilteredAdvEps, batch_size = computeAcc(imagenetteLinear, admixAdvFEps, targets, is_Vt=False)
            correctVMIWBFilteredAdvEps, batch_size = computeAcc(imagenetteLinear, vmiAdvFEps, targets, is_Vt=False)
            ...
            correctAPGDBBFilteredAdv, batch_size = computeAcc(imagenetteCNN, apgdAdvF, targets, is_Vt=False)
            correctAdmixBBFilteredAdv, batch_size = computeAcc(imagenetteCNN, admixAdvF, targets, is_Vt=False)
            correctVMIBBFilteredAdv, batch_size = computeAcc(imagenetteCNN, vmiAdvF, targets, is_Vt=False)
            ...
            correctAPGDWBFilteredAdv, batch_size = computeAcc(imagenetteLinear, apgdAdvF, targets, is_Vt=False)
            correctAdmixWBFilteredAdv, batch_size = computeAcc(imagenetteLinear, admixAdvF, targets, is_Vt=False)
            correctVMIWBFilteredAdv, batch_size = computeAcc(imagenetteLinear, vmiAdvF, targets, is_Vt=False)
