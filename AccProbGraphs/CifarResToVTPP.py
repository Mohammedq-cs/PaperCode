...
vmiAttack = VMIAttack(model=cifarResNet, beta=1.5)
...
for eps in epsilons:
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        for index, threshold in enumerate(steps):
            ...
            correctAPGDWBFilteredAdvEps, batch_size = computeAcc(cifarResNet, apgdAdvFEps, targets, is_Vt=False)
            correctAdmixWBFilteredAdvEps, batch_size = computeAcc(cifarResNet, admixAdvFEps, targets, is_Vt=False)
            correctVMIWBFilteredAdvEps, batch_size = computeAcc(cifarResNet, vmiAdvFEps, targets, is_Vt=False)
