...
loaderCifar = torch.utils.data.DataLoader(cifarDS, batch_size=256, shuffle=False, num_workers=2)

cifarResNet = getPreTrainedModel('ResNet18', 'cifar10')
filteredDS = filterDataSet(cifarResNet, loaderCifar, False)
floader = torch.utils.data.DataLoader(filteredDS, batch_size=256, shuffle=False, num_workers=2)
...
for alpha in alphas:
    apgdAccList = []
    for eps in epsilons:
        apgdM = APGDAttackModified(predict=cifarResNet, eps=eps / 255, device=device, alphaL2=alpha, is_Vt=False)
        apgdCorrect = 0
        total = 0
        apgdPestList = []
        for batch_idx, (inputs, targets) in enumerate(floader):
            ...
            correctApgdAdv, batch_size = computeAcc(cifarResNet, apgdAdv, targets)
