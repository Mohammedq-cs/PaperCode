        ...
        for i in range(self.n_iter):
            ...
            for _ in range(self.eot_iter):
                with torch.enable_grad():
                    ...
                    loss_indiv = criterion_indiv(logits, y)
                    pests = x_adv - x
                    l2Loss = torch.norm(pests.view(pests.size(0), -1), p=2, dim=1)
                    ceLoss = loss_indiv.sum()
                    loss = ceLoss + self.alphaL2 * l2Loss.sum()
                    total_loss = loss_indiv + self.alphaL2 * l2Loss
