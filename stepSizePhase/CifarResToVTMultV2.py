...
for coef in coefs:
    ...
    admixAccList = []
    vmiAccList = []
    admixAttack = AdmixAttackMult(model=cifarResNet, image_width=32, image_height=32, momentum=1.0)
    vmiAttack = VMIAttackMult(model=cifarResNet, momentum=1.0, beta=1.5)
    for eps in epsilons:
        #print("eps=", eps)
        admixCorrect = 0
        vmiCorrect = 0
