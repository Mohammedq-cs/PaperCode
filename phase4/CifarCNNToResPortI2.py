...
def maxDec_belowThreshold(input_tensor: torch.tensor, thresholdValue: float, epsValue: float) -> torch.tensor:
    ...
    random_tensor = torch.where(binary_values <= 0.5, -epsValue, epsValue).to(device)
    output_tensor = torch.where(condition, random_tensor, input_tensor)
...
for eps in epsilons:
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        # new attack
        threshold = ((np.ceil(eps / 255 / 0.01)) * 0.01) - 0.01
        admixFiltered = maxDec_belowThreshold(admixPest, threshold, eps / 255)
        vmiFiltered = maxDec_belowThreshold(vmiPest, threshold, eps / 255)
        apgdFiltered = maxDec_belowThreshold(apgdPest, threshold, eps / 255)
        ...
        # Flatten Lists
        admixPFlattened = (admixAdvFt - inputs).view(-1).tolist()
        apgdPFlattened = (vmiAdvFt - inputs).view(-1).tolist()
        vmiPFlattened = (apgdAdvFt - inputs).view(-1).tolist()
