...
for eps in epsilons:
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        # calculate Acc
        correctAPGDAdv, batch_size = computeAcc(cifarVt, apgdAdvF, targets, is_Vt=True)
        correctAdmixAdv, batch_size = computeAcc(cifarVt, admixAdvF, targets, is_Vt=True)
        correctVMIAdv, batch_size = computeAcc(cifarVt, vmiAdvF, targets, is_Vt=True)
