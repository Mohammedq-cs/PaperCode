...
floader = torch.utils.data.DataLoader(exp1filteredDS, batch_size=128, shuffle=False, num_workers=2)
...
for eps in epsilons:
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        # calculate Acc
        correctAPGDAdv, batch_size = computeAcc(resnet18Cifar, apgdAdvF, targets, is_Vt=False)
        correctAdmixAdv, batch_size = computeAcc(resnet18Cifar, admixAdvF, targets, is_Vt=False)
        correctVMIAdv, batch_size = computeAcc(resnet18Cifar, vmiAdvF, targets, is_Vt=False)
