...
loaderIN = torch.utils.data.DataLoader(imagenetteDS, batch_size=30, shuffle=False, num_workers=2)
...
floader = torch.utils.data.DataLoader(filteredDS, batch_size=30, shuffle=False, num_workers=2)
...
for eps in epsilons:
    ...
    for batch_idx, (inputs, targets) in enumerate(floader):
        ...
        # calculate Acc
        correctAPGDAdv, batch_size = computeAcc(imagenetteCNN, apgdAdvF, targets, is_Vt=False)
        correctAdmixAdv, batch_size = computeAcc(imagenetteCNN, admixAdvF, targets, is_Vt=False)
        correctVMIAdv, batch_size = computeAcc(imagenetteCNN, vmiAdvF, targets, is_Vt=False)
        correctMaxDecAdv, batch_size = computeAcc(imagenetteCNN, maxDecAdv, targets, is_Vt=False)
